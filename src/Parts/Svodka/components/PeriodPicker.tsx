...
export function PeriodPicker({ months, period, onChange }: PeriodPickerProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-2.5 gap-y-2">
      ...
      <div
        className="flex flex-wrap gap-1 rounded-lg border border-rule p-1"
        role="group"
        aria-label="Tayyor davrlar"
      >
        {PRESETS.map((p) => {
          ...
          return (
            <button
              ...
              className={
                ...
                (on
                  ? "bg-[color-mix(in_srgb,var(--s1)_22%,transparent)] font-semibold text-ink"
