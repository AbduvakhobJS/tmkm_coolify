...
export function UnverifiedBanner({ area, extra }: { area: string; extra?: ReactNode }) {
  ...
  return (
    <Banner tone="crit">
      <b>Ma'lumot tekshirilmoqda — son qiymatlar ko'rsatilmayapti.</b> {reason} Bo'lim tuzilishi,
      voqealar ro'yxati va sanalar manbadagi ko'rinishida qoldirilgan.{extra ? <> {extra}</> : null}
    </Banner>
  );
}
...
export function MaskedChart({ area, what }: { area: string; what: string }) {
  ...
  return (
    <EmptyState
      title="Diagramma chizilmadi"
      text={
        <>
          {what} — manba qiymatlari tekshirilmagani uchun grafik ko'rsatilmayapti: noto'g'ri shakl ham
          noto'g'ri xulosaga olib keladi. {reason}
        </>
      }
    />
  );
