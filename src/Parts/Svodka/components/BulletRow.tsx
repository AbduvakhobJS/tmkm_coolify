...
export function BulletRow({ label, plan, fact, unit, src }: BulletDatum) {
  ...
  const sub = `fakt ${exact(fact)} · reja ${exact(plan)} ${unit ?? ""}`.trim();

  const title =
    `${label}\nReja: ${exact(plan)} ${unit ?? ""}\nFakt: ${exact(fact)} ${unit ?? ""}` +
    `\nBajarilish: ${pc === null ? "—" : nf(pc, 1) + "%"}` +
    (src ? `\nManba satri: №${src}` : "");
...
export function BulletChart({ rows }: { rows: (BulletDatum & { key: string })[] }) {
  if (rows.length === 0) {
    return <div className="px-2.5 py-6 text-center text-[13px] text-ink-3">Ma'lumot yo'q.</div>;
  }
  return (
    <div>
      {rows.map(({ key, ...row }) => (
  ...
  return (
    <div className="mb-2 flex flex-wrap items-center gap-x-3.5 gap-y-1.5 text-[11.5px] text-ink-2">
      ...
      <span className="text-ink-3">
        o'ngdagi foiz rangi: yashil ≥100% · sariq 85–99% · qizil &lt;85%; har bir qator o'z shkalasida
      </span>
    </div>
  );
}
