...
import { dateLabel, dateTick, monthLabel, monthTick, unitUz } from "../format";
...
export function unitGroups(rows: ProdItem[]): UnitGroup[] {
  ...
  return [...byUnit.entries()]
    .map(([unit, list]) => ({
      unit,
      name: unit === "—" ? "Birligi aniqlanmagan" : `O'lchov birligi: ${unitUz(unit)}`,
      items: list
...
export function monthlyTrend(rows: ProductionMonthlyRow[], months: string[]): MonthlyTrendVM {
  ...
  // `"тн"` — API'даги қиймат (кирилл ёзувида). Бу ерда у **фильтр калити**:
  // қуйида шу бирликдаги қаторлар ажратилади. Шунинг учун таржима қилинмайди;
  // экранга чиқарилаётганда `unitUz()` дан ўтказилади.
  const unit =
