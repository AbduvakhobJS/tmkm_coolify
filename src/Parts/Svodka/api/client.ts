...
const API_BASE = String(
  process.env.REACT_APP_PRODUCTION_API_BASE ?? "https://tmk.bgs.uz/api/production-report",
).replace(/\/+$/, "");
...
export class NotAvailableError extends ApiError {
  constructor(path: string) {
    super(`Bo'lim serverda mavjud emas: ${path}`, 404);
...
export class UnauthorizedError extends ApiError {
  constructor() {
    super("Seans muddati tugadi. Xost tizimiga qayta kiring.", 401);
...
export class NetworkError extends ApiError {
  constructor() {
    super("Server bilan bog'lanib bo'lmadi. Tarmoqni tekshiring.", 0);
...
export async function apiGet<T>(
  ...
  if (res.status === 403) throw new ApiError("Ushbu bo'limga ruxsat yo'q (403).", 403);
  if (!res.ok) throw new ApiError(`Server xatosi (${res.status}).`, res.status);

  try {
    return (await res.json()) as T;
  } catch {
    throw new ApiError("Server javobini o'qib bo'lmadi.", res.status);
  }
}
