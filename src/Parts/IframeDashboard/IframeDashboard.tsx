...
import { IFRAME_NAV_ITEMS } from '../../components/MetalsDashboardMain';
import './IframeDashboard.css';

/**
 * `MetalsDashboardMain.tsx` dagi "Batafsil" tugmasi shu BITTA komponentga
 * olib keladi (`/main/iframe/:key`) — sahifaning o'zi almashmaydi, faqat
 * tashqi dashboard manzilining `#<key>` qismi o'zgaradi:
 *
 *   Ishlab chiqarish bosilsa → .../?from=...&to=...#prod
 *   Elektr energiya bosilsa  → .../?from=...&to=...#energy
 *   ...va h.k.
 *
 * Yuqoridagi navbar band ro'yxati (`IFRAME_NAV_ITEMS`) `MetalsDashboardMain.tsx`
 * da e'lon qilingan — yangi bo'lim qo'shilganda faqat o'sha massivni
 * o'zgartirish kifoya, bu yerga tegilmaydi.
 */
...
const IframeDashboard: React.FC = () => {
    ...
    const isValidKey = IFRAME_NAV_ITEMS.some((item) => item.path === `/${key}`);
    ...
    return (
        <div className="ifd-page">
            <nav className="ifd-nav">
                {IFRAME_NAV_ITEMS.map((item) => {
                    const itemKey = item.path.slice(1);
                    return (
                        <button
                            key={item.path}
                            type="button"
                            className={`ifd-nav-item${itemKey === key ? ' active' : ''}`}
                            onClick={() => navigate(`/main/iframe${item.path}`)}
                        >
                            {item.label}
                        </button>
                    );
