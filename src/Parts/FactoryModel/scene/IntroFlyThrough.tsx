 ...
 * One-shot cinematic played when the factory scene first mounts: eases the
 * camera down from the default overview to walking height, tours the closed
 * 20-point loop around the model over INTRO_LOOP_DURATION seconds — always
 * looking at the model's center — then eases back up to the overview before
 * handing control to OrbitControls.
 *
 * IMPORTANT: while introPlaying is true, OrbitControls is `enabled={false}`
 * (see FactoryScene), which means its internal update() loop does NOT
 * reorient the camera even if we mutate controls.target. So during the
 * intro we must drive camera orientation ourselves via camera.lookAt(),
 * independent of OrbitControls. We still keep controls.target in sync so
 * that OrbitControls picks up smoothly the moment it takes back control.
 */
const IntroFlyThrough: React.FC<IntroFlyThroughProps> = ({ controlsRef, onDone }) => {
    const { camera } = useThree();
    ...
    const state = useRef({
        ...
        startPos: new THREE.Vector3(...CAMERA_INITIAL_POSITION),
        startTarget: new THREE.Vector3(0, 0, 0),
        // Camera always looks here — the model's center.
        lookCenter: new THREE.Vector3(0, INTRO_WALK_HEIGHT * 0.5, 0),
    }).current;
    ...
    useEffect(() => {
        camera.position.copy(state.startPos);
        camera.lookAt(state.startTarget);
        controlsRef.current?.target.copy(state.startTarget);
    }, [camera, controlsRef, state]);
    ...
    useFrame((_, delta) => {
        ...
        if (state.phase === "descend") {
            ...
            const ease = 1 - Math.pow(1 - state.t, 3);
            camera.position.lerpVectors(state.startPos, curve.getPointAt(0), ease);
            scratchTarget.lerpVectors(state.startTarget, state.lookCenter, ease);
        ...
        if (state.phase === "loop") {
            ...
            curve.getPointAt(state.t, scratchPos);
            camera.position.copy(scratchPos);
            camera.lookAt(state.lookCenter);
            controls.target.copy(state.lookCenter);
            if (state.t >= 1) {
                state.phase = "return";
                state.t = 0;
            }
        ...
        const ease = 1 - Math.pow(1 - state.t, 3);
        camera.position.lerpVectors(curve.getPointAt(0), state.startPos, ease);
        scratchTarget.lerpVectors(state.lookCenter, state.startTarget, ease);
        camera.lookAt(scratchTarget);
        controls.target.copy(scratchTarget);
        if (state.t >= 1) {
            state.phase = "done";
