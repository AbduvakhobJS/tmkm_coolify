...
const MetalsDashboardMain: React.FC = () => {
    ...
    return (
        <DashRoot>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 8, flexShrink: 0 }}>
                <KpiCard title="Umumiy hajmi" value={`${fmt(TOTAL)} t`} delta={7.3} compare={COMPARE}
                ...
                {METALS.map((m) => (
                    <KpiCard key={m.name} title={m.name} value={`${fmt(m.value)} t`}
